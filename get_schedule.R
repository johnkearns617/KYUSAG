...
### Read in ID's table
teamid <- 
  read.csv("Info/conferences.csv", as.is = T) %>%
...
if("USC Upstate" %!in% z$team){
  test = z %>% 
    filter(opponent=="USC Upstate") %>% 
    rename(team=opponent,
           opponent=team,
           teamscore=oppscore,
           oppscore=teamscore) %>% 
    mutate(location = ifelse(location=="H","A",location))
  
  z = rbind(z,test)
}

### Save Results
write.csv(z, paste("Results/2020-21/NCAA_Hoops_Results_", month, "_", 
